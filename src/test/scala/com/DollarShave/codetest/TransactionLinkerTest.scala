...
"a8062dd2-a0c2-4404-86b9-1ca31ce172e7,2016-01-02T04:35:40.000+00:00,c5f2f32873e8e100,/your-box/funnel")
  ...
  "parseUsersFromCSV" should "convert an RDD of string an RDD of (userId, user)" in {
    ...
    events.head shouldEqual DSCEvent("babbe8fe-5dd0-4c53-b1ea-4826095ac925",
      DSCEvent.parseDate("2016-01-01T01:30:55.000+00:00"),
      "fee830db7fd1554b",
      "/blades",
      "")
    events(1) shouldEqual DSCEvent(
      "d136e575-528e-45d6-8922-e6222b14dfae",
      DSCEvent.parseDate("2016-01-01T01:31:32.000+00:00"),
      "fee830db7fd1554b",
      "/your-account",
      "")
  }
  ...
  "getTimeStamps" should "write timestamps for each user" in {
    ...
    val releventInfo = answer.map(_.split(",")).map(t => (t(0), DSCEvent.parseDate(t(1)),DSCEvent.parseNextId(t.toList)))
